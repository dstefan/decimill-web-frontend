...
/**
 * Encodes integers up to 8,999,999.
 * 
 * @param int $ID
 * @return string
 */
function encode($ID) {
    $table = [
        'cJQtXdAipz', 'FOhnvSgDUx', 'alKsEWwkLo', 'ZemHRYfuTj', 'BbVqyPMrGl',
        'NqueGXbKmo', 'cLStdIWhpa', 'PUglQCkwEJ', 'iOZnHrVMzD', 'AjyBFvxYRz'
    ];
    $encID = '';
    $ID = $ID + 1000000;
    for ($i = 0; $i < 7; $i++) {
...
/**
 * The inverse for encode (integers up to 8,999,999).
 * 
 * @param string $ID
 * @return int
 */
function decode($ID) {
    ...
    for ($i = 0; $i < 7; $i++) {
        $char = substr($ID, $i, 1);
        $num = strpos($table[$i], $char);
        if ($num === FALSE) {
            return FALSE;
        }
        $decID .= $num;
    }
    return $decID - 1000000;
}
...
function throwErrorAndExit($message) {
    ...
    if (key_exists('ajax', $_GET)) {
        echo $err->show_ajax_error($message);
    } else {
        echo $err->show_error($message, $header);
    }
...
function is_mobile() {
    ...
    return strpos($ac, 'application/vnd.wap.xhtml+xml') !== false || $op != '' ||
            strpos($ua, 'sony') !== false || strpos($ua, 'symbian') !== false ||
            strpos($ua, 'nokia') !== false || strpos($ua, 'samsung') !== false ||
            strpos($ua, 'mobile') !== false || strpos($ua, 'windows ce') !== false ||
            strpos($ua, 'epoc') !== false || strpos($ua, 'opera mini') !== false ||
            strpos($ua, 'nitro') !== false || strpos($ua, 'j2me') !== false || strpos($ua, 'midp-') !==
            false || strpos($ua, 'cldc-') !== false || strpos($ua, 'netfront') !==
            false || strpos($ua, 'mot') !== false || strpos($ua, 'up.browser') !==
            false || strpos($ua, 'up.link') !== false || strpos($ua, 'audiovox') !==
            false || strpos($ua, 'blackberry') !== false || strpos($ua, 'ericsson,') !==
            false || strpos($ua, 'panasonic') !== false || strpos($ua, 'philips') !==
            false || strpos($ua, 'sanyo') !== false || strpos($ua, 'sharp') !== false ||
            strpos($ua, 'sie-') !== false || strpos($ua, 'portalmmm') !== false ||
            strpos($ua, 'blazer') !== false || strpos($ua, 'avantgo') !== false ||
            strpos($ua, 'danger') !== false || strpos($ua, 'palm') !== false || strpos($ua, 'series60') !==
            false || strpos($ua, 'palmsource') !== false || strpos($ua, 'pocketpc') !==
            false || strpos($ua, 'smartphone') !== false || strpos($ua, 'rover') !==
            false || strpos($ua, 'ipaq') !== false || strpos($ua, 'au-mic,') !==
            false || strpos($ua, 'alcatel') !== false || strpos($ua, 'ericy') !==
            false || strpos($ua, 'up.link') !== false || strpos($ua, 'vodafone/') !==
            false || strpos($ua, 'wap1.') !== false || strpos($ua, 'wap2.') !== false;
}
